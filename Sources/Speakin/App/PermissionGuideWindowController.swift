...
为了监听 Fn 键并将语音文字粘贴到当前输入框，Speakin 需要"辅助功能"权限。

操作步骤：
1) 点击下方"打开系统设置"
2) 在"隐私与安全性 → 辅助功能"列表中找到 Speakin
3) 如果 Speakin 已存在但检测失败：先点"-"移除，再点"+"重新添加并开启
...
"""
            setStatusBanner(icon: "🎉", text: "所有配置已完成，Speakin 已就绪！", color: NSColor.systemGreen.withAlphaComponent(0.15), textColor: .systemGreen)
    ...
    func setRecheckFailedHint(_ message: String? = nil) {
        setStatusBanner(
            icon: "⚠️",
            text: message ?? "仍未授权，请确认 Speakin 开关已打开。",
